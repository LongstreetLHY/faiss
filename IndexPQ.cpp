...
void IndexPQ::reconstruct_n (idx_t i0, idx_t ni, float *recons) const
{
    ...
    for (idx_t i = 0; i < ni; i++) {
        const uint16_t * code = &codes[(i0 + i) * pq.code_size];
                           ...
                           float *distances, idx_t *labels) const
{
    ...
    if (search_type == ST_PQ) {  // Simple PQ search
        ...
        uint16_t * q_codes = new uint16_t [n * pq.code_size];
        ScopeDeleter<uint16_t> del (q_codes);
        ...
        if (!encode_signs) {
            ...
            for (size_t i = 0; i < n; i++) {
                ...
                uint16_t *code = q_codes + i * pq.code_size;
                for (int j = 0; j < d; j++)
...
static size_t polysemous_inner_loop (
        const IndexPQ & index,
        const float *dis_table_qi, const uint16_t *q_code,
        size_t k, float *heap_dis, long *heap_ids)
{
    ...
    const uint16_t *b_code = index.codes.data();
                                          ...
                                          float *distances, idx_t *labels) const
{
    ...
    uint16_t * q_codes = new uint16_t [n * pq.code_size];
    ScopeDeleter<uint16_t> del2 (q_codes);
...
#pragma omp parallel for reduction (+: n_pass)
    for (idx_t qi = 0; qi < n; qi++) {
        const uint16_t * q_code = q_codes + qi * pq.code_size;
                                          ...
                                          long *hist)
{
    ...
    uint16_t * b_codes ;
    ScopeDeleter <uint16_t> del_b_codes;

    if (xb) {
        b_codes = new uint16_t [nb * pq.code_size];
...
void MultiIndexQuantizer::reconstruct (idx_t key, float * recons) const
{
    if (pq.byte_per_idx == 1) {
        ...
        pq.decode ((uint16_t*)code, recons);
