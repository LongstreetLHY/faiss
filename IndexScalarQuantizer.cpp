...
struct Codec8bit {
    ...
    static void encode_component (float x, uint16_t *code, int i) {
        code[i] = (int)(255 * x);
    }

    static float decode_component (const uint16_t *code, int i) {
        return (code[i] + 0.5f) / 255.0f;
    }

#ifdef USE_AVX
    static __m256 decode_8_components (const uint16_t *code, int i) {
...
struct Quantizer {
    virtual void encode_vector(const float *x, uint16_t *code) const = 0;
    virtual void decode_vector(const uint16_t *code, float *x) const = 0;

    virtual float compute_distance_L2 (SimilarityL2 &sim,
                                       const uint16_t * codes) const = 0;
...
struct QuantizerUniform: Quantizer {
    ...
    void encode_vector(const float* x, uint16_t* code) const override {
    ...
    void decode_vector(const uint16_t* code, float* x) const override {
      for (size_t i = 0; i < d; i++) {
    ...
    float reconstruct_component (const uint16_t * code, int i) const
    {
        float xi = Codec::decode_component (code, i);
...
#ifdef USE_AVX
    __m256 reconstruct_8_components (const uint16_t * code, int i) const
    {
    ...
    float compute_distance_L2(SimilarityL2& sim, const uint16_t* codes)
        const override {
      return compute_distance(*this, sim, codes);
    }

    float compute_distance_IP(SimilarityIP& sim, const uint16_t* codes)
        const override {
      return compute_distance(*this, sim, codes);
    }
};
...
void ScalarQuantizer::decode (const uint16_t *codes, float *x, size_t n) const
{
    Quantizer *squant = select_quantizer (*this);
#pragma omp parallel for
    for (size_t i = 0; i < n; i++)
                            ...
                            int k, float *simi, idx_t *idxi)
{
    ...
    for (int i = 0; i < nprobe; i++) {
        ...
        const uint16_t* codes = index.codes[list_no].data();
...
void IndexIVFScalarQuantizer::merge_from_residuals (IndexIVF & other_in) {
    ...
    for (int i = 0; i < nlist; i++) {
        std::vector<uint16_t> & src = other.codes[i];
        std::vector<uint16_t> & dest = codes[i];
