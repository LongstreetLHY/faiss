...
static const uint16_t hamdis_tab_ham_bytes[256] = {
...
void fvecs2bitvecs (const float * x, uint16_t * b, size_t d, size_t n)
{
...
#pragma omp parallel for
    for (int i = 0; i < na; i++) {
        const uint16_t *ca = a + i * code_size;
        const uint16_t *cb = b;
