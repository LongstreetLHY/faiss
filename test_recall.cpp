...
int main(int argc, char *argv[])
{
    ...
    //faiss::index *index;
    faiss::IndexIVFPQ *index = new faiss::IndexIVFPQ(&coarse_quantizer, d,
                             ncentroids, M, nbits_per_idx);	
    ...
    { // Use the found configuration to perform a search
        ...
        std::ofstream data("Recall_data.txt",std::ios::app);
		if(data.is_open())
		{
			data<<M<<" "<<nbits_per_idx<<" "<<n_1/float(nq)<<" "<<n_10/float(nq)<<" "<<n_100/float(nq)<<" "<<"\n";
			data.close();
		}
