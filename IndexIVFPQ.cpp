...
{
    FAISS_THROW_IF_NOT (nbits_per_idx <= 14);
...
void IndexIVFPQ::train_residual_o (idx_t n, const float *x, float *residuals_2)
{
    ...
    if (residuals_2) {
        uint16_t *train_codes = new uint16_t [pq.code_size * n];
        ScopeDeleter<uint16_t> del (train_codes);
...
void IndexIVFPQ::encode (long key, const float * x, uint16_t * code) const
{
    if (by_residual) {
...
void IndexIVFPQ::decode_multiple (size_t n, const long *keys,
                                  const uint16_t * xcodes, float * x) const
{
...
{
    printf("go to add_core_o() in IndexIVFPQ::add_with_ids\n");
                             ...
                             float *residuals_2, const long *precomputed_idx)
{
    ...
    if (precomputed_idx) {
        ...
        printf("finish quantizer assign\n");
        idx = idx0;
    }

    double t1 = getmillisecs ();
    uint16_t * xcodes = new uint16_t [n * code_size];
    printf("code_size = %lu\n", code_size);
    ScopeDeleter<uint16_t> del_xcodes (xcodes);

    const float *to_encode = nullptr;
    ScopeDeleter<float> del_to_encode;

    printf("go to compute_residual\n");
    if (by_residual) {
        float *residuals = new float [n * d];
        // TODO: parallelize?
        for (size_t i = 0; i < n; i++) {
    ...
    printf("go to pq.computer_codes\n");
    pq.compute_codes (to_encode, xcodes, n);
    printf("finish pq.computer_codes\n");
    ...
    for (size_t i = 0; i < n; i++) {
       ...
       // printf("memset residual_2\n");
        idx_t id = xids ? xids[i] : ntotal + i;
        ids[key].push_back (id);
        uint16_t *code = xcodes + i * code_size;
        //printf("new uint16_t code\n");
        for (size_t j = 0; j < code_size; j++)
            codes[key].push_back (code[j]);
        //printf("finish push_back codes[key]\n");
        if (residuals_2) {
            float *res2 = residuals_2 + i * d;
            const float *xi = to_encode + i * d;
            pq.decode (code, res2);
            //printf("finish pq.decode\n");
            for (int j = 0; j < d; j++)
                res2[j] = xi[j] - res2[j];
        }
    ...
    printf("finish direct_map circulation\n");
    ...
    ntotal += n;
    printf("end of add_core_o function\n");
}
...
void IndexIVFPQ::reconstruct_n (idx_t i0, idx_t ni, float *recons) const
{
    ...
    for (int key = 0; key < nlist; key++) {
        ...
        const uint16_t * code_line = codes[key].data();
...
void IndexIVFPQ::reconstruct (idx_t key, float * recons) const
{
    ...
    const uint16_t * code = &(codes[list_no][ofs * pq.code_size]);
                     ...
                     long a1, long a2) const
{
    ...
    for (long list_no = 0; list_no < nlist; list_no++) {
        ...
        const std::vector<uint16_t> & codes_in = codes[list_no];
        std::vector<uint16_t> & codes_out = other.codes[list_no];
...
struct InvertedListScanner: QueryTables {
    ...
    const uint16_t * __restrict list_codes;
             ...
             size_t k, float * heap_sim, long * heap_ids, bool store_pairs)
    {
        ...
        for (size_t j = 0; j < list_size; j++) {
            const uint16_t *b_code = list_codes;
...
#pragma omp  for
        for (size_t i = 0; i < nx; i++) {
            ...
            //printf("finish maxheap_heapify %lu\n", i);
            heap_cycles += TOC;

            TIC;
            qt.init_query (qi);
            //printf("finish init_query %lu\n", i);
...
#pragma omp critical
        {
            ...
            //printf("Doing critical conditions\n");
        }
                              ...
                              float *distances, idx_t *labels) const
{
    ...
    uint64_t t0;
    TIC;
    printf("goto quantizer->search\n");
    quantizer->search (n, x, nprobe, coarse_dis, idx);
    indexIVFPQ_stats.assign_cycles += TOC;

    TIC;
    float_maxheap_array_t res = { size_t(n), size_t(k), labels, distances};
    printf("go to search_knn_with_key\n");
    search_knn_with_key (n, x, idx, coarse_dis, &res);
    printf("finish search_knn_with_key\n");
    printf("go to search_cycles\n");
...
#pragma omp for
        for (idx_t i = 0; i < n; i++) {
            ...
            for (int j = 0; j < k_coarse; j++) {
                ...
                // 2nd level residual
                const uint16_t * l2code = &codes[list_no][ofs * pq.code_size];
...
IndexIVFPQCompact::IndexIVFPQCompact (const IndexIVFPQ &other)
{
    ...
    compact_codes = new uint16_t [ntotal * code_size];
...
void IndexIVFPQCompact::search_knn_with_key (
    ...
    bool store_pairs) const
{
    printf("succefully call function\n");
    const size_t k = res->k;
