...
void ProductQuantizer::compute_code (const float * x, uint16_t * code)  const
{
    ...
    for (size_t m = 0; m < M; m++) {
        ...
        switch (byte_per_idx) {
          case 1:  code[m] = (uint16_t) idxm;  break;
...
void ProductQuantizer::decode (const uint16_t *code, float *x) const
{
    if (byte_per_idx == 1) {
        for (size_t m = 0; m < M; m++) {
...
void ProductQuantizer::decode (const uint16_t *code, float *x, size_t n) const
{
...
void ProductQuantizer::compute_codes (const float * x,
                                      ...
                                      size_t n)  const
{
    printf("pq.code_size = %lu\n",code_size);
    printf("dsub = %lu\n", dsub);
    if (dsub <= 16) { // simple direct computation
    printf("go to compute_code\n");
#pragma omp parallel for num_threads(1)
        ...
        printf("go to compute_distance_tables using blas\n");
        float *dis_tables = new float [n * ksub * M];
        ScopeDeleter<float> del (dis_tables);
        compute_distance_tables (n, x, dis_tables);
    printf("finish compute_code or computer_distance_tables\n");

#pragma omp parallel for num_threads(1)
        for (size_t i = 0; i < n; i++) {
            uint16_t * code = codes + i * code_size;
...
void ProductQuantizer::search_sdc (const uint16_t * qcodes,
                     size_t nq,
                     const uint16_t * bcodes,
                     const size_t nb,
                     float_maxheap_array_t * res,
                     bool init_finalize_heap) const
{
...
#pragma omp parallel for
    for (size_t i = 0; i < nq; i++) {
        ...
        const uint16_t * qcode = qcodes + i * code_size;

        if (init_finalize_heap)
            maxheap_heapify (k, heap_dis, heap_ids);

        const uint16_t * bcode = bcodes;
